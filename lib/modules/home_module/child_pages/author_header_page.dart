...
class _AuthorHeaderPageState extends State<AuthorHeaderPage> with SingleTickerProviderStateMixin {
  ...
  _kudosAction(){
    ...
    if (widget.videoInfo.isLike){
      UUHomeDao.kudosRequest(widget.videoInfo.vid!,isCancel: true)
          .then((value){
      ...
      UUHomeDao.kudosRequest(widget.videoInfo.vid!).then((value){
        widget.videoInfo.like+=1;
        showToast(value.msg);
        setState(() {
  ...
  _favoriteAction(){
    if (widget.videoInfo.isFavorite){
      UUHomeDao.favoriteRequest(widget.videoInfo.vid!,isFavoriteRequest: false).then((model){
      ...
      UUHomeDao.favoriteRequest(widget.videoInfo.vid!).then((mode){
        widget.videoInfo.favorite+=1;
        showToast(mode.msg);
        setState(() {
          widget.videoInfo.isFavorite  = !widget.videoInfo.isFavorite;
